...
#  2021-03-02 ver.6 cut_bandを複数指定可能にして、且つ基準を上端に取ったり下端に捕ることを可能にした。
...
def get_melspectrogram_image(data, sr, n_mels=128, fmax=10000, n_fft=2048, hop_length=512, top_remove=0, emphasize_band=None, cut_band=None, raw=False):
    ...
    """
    S = librosa.feature.melspectrogram(data, sr=sr, n_mels=n_mels, fmax=fmax, n_fft=n_fft, hop_length=hop_length)  # スペクトログラムのデータを取得（2次元のndarray型）
...
def save_spectrogram_with_window(data, params, rp0=0, terminal=None):
    ...
    while True:
        ...
        if len(sliced_data) < ep - rp:  # この条件文は現状では実行されない気がするが、バグを作りこんだとき用に残しておく
        ...
        path = "{3}__{0}__{1:.1f}_{2:.1f}.png".format(tag, t_start, t_width, msg)
        if root != "":
            path = root + "/" + path
        im.save(path)
...
def save_spectrogram_with_timelist(file_path: str, loc: str, params: dict):
    ...
    y, sr = librosa.load(file_path, sr=sr, res_type=load_mode)  # sampling frequency == sampling rate
...
def get_location_ID(file_path):
    ...
    if location == "unknown":
        ...
        location = str(len(dir_name)) + hs[:2]
