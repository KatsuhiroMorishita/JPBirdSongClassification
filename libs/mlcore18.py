...
#  2022-11-06 ver.14 image_preprocessing12.pyに変更
#  2022-12-20 ver.14 image_preprocessing13.pyに変更
...
def gen_custom_loss(func_name: str, param: dict={}):
    """ 独自定義の損失関数を作って返す
    func_name: str, 作成する関数の名称
    param: dict, 作成する関数のパラメータ
    """

    if func_name == "custom_loss1":
        ...
        def focal_loss(y_true, y_pred):
            """ Focal Lossを定義
            gammaが0.5のときはtf.math.sqrtを使った方が計算が速いかも
            """
            y_pred = tf.math.maximum(y_pred, 0.0001) 
            y_pred = tf.math.minimum(y_pred, 0.9999)
    ...
    print("warning: 該当する関数がありませんでした。設定を確認してください。")
    time.sleep(5)
...
def plot_history(history, show_flag=True, save_name="", log_scale=True):
    ...
    if save_name != "":
        ...
        plt.savefig(os.path.join(dir_name, "loss_" + file_name))
    if show_flag:
        plt.show()
    ...
    plt.plot(x, y1, "o-", label="accuracy")
    plt.plot(x, y2, "^-", label="val_accuracy")
    ...
    if save_name != "":
        dir_name, file_name = os.path.split(save_name)
        plt.savefig(os.path.join(dir_name, "acc_" + file_name))
    if show_flag:
        plt.show()
...
def save_history(fname, history, mode="a"):
    ...
    df_["accuracy"] = history.history['accuracy']
    df_["val_accuracy"] = history.history['val_accuracy']
...
def reload(load_names=['x_train.npy', 'y_train.npy', 'x_test.npy', 'y_test.npy', 'weights_dict.pickle', 'label_dict.pickle'], 
    ...
    if with_model:
        model_name = f"model{model_format}"
